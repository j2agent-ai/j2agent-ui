...
/** xychart 柱/条色板（与 pie1–pie8 一致，横竖向共用） */
const XYCHART_BAR_FILL_PALETTE = [
	'#93C5FD',
	'#FDE68A',
	'#86EFAC',
	'#C4B5FD',
...
const XYCHART_PLOT_COLOR_PALETTE = XYCHART_BAR_FILL_PALETTE.join(',')
const MERMAID_DIAGRAM_FONT_FAMILY =
...
const measureSvgTextWidth = (text: string, fontSize: number) => {
	...
	ctx.font = `${fontSize}px ${MERMAID_DIAGRAM_FONT_FAMILY}`
...
const applyXychartBarColors = (svg: SVGElement) => {
	svg.querySelectorAll('g[class*="bar-plot"]').forEach((group) => {
		...
		const paletteIndex = indexMatch
			...
			: 0
		const fill =
			XYCHART_BAR_FILL_PALETTE[
				paletteIndex % XYCHART_BAR_FILL_PALETTE.length
			]
...
/** Mermaid 彩色浅色系主题配置，统一字体、节点、连线与饼图色板。 */
const MERMAID_MINIMAL_THEME_VARIABLES = {
	fontFamily:
		'-apple-system, BlinkMacSystemFont, "Segoe UI", "PingFang SC", "Hiragino Sans GB", "Microsoft YaHei", sans-serif',
	fontSize: '13px',
	textColor: '#334155',
...
/** xychart 主题（横/竖向同一套，避免仅 vertical 走 Mermaid 默认紫系） */
const MERMAID_XYCHART_THEME_VARIABLES = {
	plotColorPalette: XYCHART_PLOT_COLOR_PALETTE,
...
const MERMAID_THEME_VARIABLES = {
	...MERMAID_MINIMAL_THEME_VARIABLES,
	xyChart: MERMAID_XYCHART_THEME_VARIABLES
} as const

/** Mermaid 的极简样式补丁，增强图例与标签可读性。 */
const MERMAID_MINIMAL_THEME_CSS = `
	.legend text,
	.legendText,
	.slice text,
	.pieTitleText {
...
const getMermaidApi = async () => {
	if (!mermaidApi) {
		...
		mermaidApi.initialize({
			...
			theme: 'default',
			themeVariables: MERMAID_THEME_VARIABLES as unknown as Record<
				string,
				unknown
			>,
			themeCSS: MERMAID_MINIMAL_THEME_CSS,
